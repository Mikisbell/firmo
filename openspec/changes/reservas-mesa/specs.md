# Reservas de Mesa — Specification
...
|--------|------|-------------|-----------|
| reservations (public API) | New | 4 (creation, query, cancel, rate limit) | 12 |
| reservations (admin API) | New | 2 (listing, actions) | 7 |
| reservations (business rules) | New | 3 (validation, availability, confirmation code) | 10 |
| reservations (events) | New | 1 (event store) | 1 |
| reservations (UI) | New | 2 (public page, admin page) | 4 |
| reservations (DB) | New | 1 (migration) | 1 |
| **Total** | | **13 requirements** | **35 scenarios** |

### Coverage
- Happy paths: Covered (creation, query, cancel, admin listing, admin actions, UI flows)
- Edge cases: Covered (past dates, max party size, out-of-hours, invalid slug, ambiguous codes)
- Error states: Covered (overbooking, concurrency, rate limiting, invalid transitions, auth failure)
- Concurrency: Covered (two simultaneous requests for last table)
- Security: Covered (rate limiting, no data leaks, auth on admin, tenant isolation)
