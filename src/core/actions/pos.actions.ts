...
export const POSActions = {
  ...
  async createOrder(
    ...
    const fulfillment =
      params.fulfillment ||
      (params.table_number ? { table_number: params.table_number } : undefined);
      ...
      payload: {
        ...
        delivery: params.delivery
          ? {
              ...
              payment_expectation:
                params.delivery.payment_expectation ?? "PREPAID",
    ...
    await appendEvent(tenant_id, terminal_id, {
      ...
      payload: {
        ...
        line: {
          ...
          mods: [],
          is_special_sla: item.is_special_sla ?? false,
        },
      },
    });
  ...
  /**
   * Acknowledge check request for a table (waiter action)
   */
  async acknowledgeCheckRequest(
    tenant_id: string,
    terminal_id: string,
    actor_id: string,
    order_id: string,
    ...
    await appendEvent(tenant_id, terminal_id, {
      event_id: newUUID(),
      event_type: "CHECK_REQUEST_ACKNOWLEDGED",
      aggregate_type: "ORDER",
      aggregate_id: order_id,
      correlation_id: order_id,
      causation_id: null,
  ...
  async issueRefund(
    ...
    { success: true; refund_id: string } | { success: false; error: string }
  > {
    try {
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Error al emitir reembolso",
      };
    }
  },
  ...
  async attachTable(
    ...
    await appendEvent(tenant_id, terminal_id, {
      ...
      payload: {
        order_id,
        primary_table_id: primary_table,
        attached_table_id,
  ...
  async getNextOrderNumber(
    ...
    { success: true; orderNumber: number } | { success: false; error: string }
  > {
    try {
      // Try to get current range
      const res = await fetch(
        `/api/terminals/range?terminal_id=${encodeURIComponent(terminal_id)}&tenant_id=${encodeURIComponent(tenant_id)}`,
      );
      ...
      if (!res.ok) {
        return {
          success: false,
          error: data.error || "Error al obtener número de orden",
        };
      }
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Error de red al obtener número de orden",
      };
    }
  },
};
