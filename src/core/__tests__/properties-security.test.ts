    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (user, role, operation) => {
          ...
          if ((operation as any).type === 'csv_import') {
            if (hasAdminRole) {
      ...
      fc.asyncProperty(
        ...
        async (userTenant, productTenant, products, updateRequest) => {
          ...
          mockBulkService.bulkUpdate.mockImplementation(async (ids, updates, tenant, user) => {
            ...
            const actualIds = ids.filter((id: string) => allowedIds.includes(id));

            return {
              success_count: actualIds.length,
              failure_count: ids.length - actualIds.length,
              failures: ids
                .filter((id: string) => !allowedIds.includes(id))
                .map((id: string) => ({
                  product_id: id,
                  sku: 'unknown',
