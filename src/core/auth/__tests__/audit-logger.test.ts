...
describe('Audit Logger Service', () => {
  ...
  describe('logAuthEvent', () => {
    it('should log a complete authentication event with all fields', async () => {
      ...
      vi.mocked(prisma.auth_events.create).mockResolvedValue(mockCreatedEvent as any);
  ...
  describe('createSecurityAlert', () => {
    it('should create a security alert with all required fields', async () => {
      ...
      vi.mocked(prisma.security_alerts.create).mockResolvedValue(mockCreatedAlert as any);
  ...
  describe('queryEvents', () => {
    it('should query events with all filters applied', async () => {
      ...
      vi.mocked(prisma.auth_events.findMany).mockResolvedValue(mockEvents as any);
  ...
  describe('queryAlerts', () => {
    it('should query alerts with all filters applied', async () => {
      ...
      vi.mocked(prisma.security_alerts.findMany).mockResolvedValue(mockAlerts as any);
  ...
  describe('acknowledgeAlert', () => {
    it('should acknowledge an alert with employee ID and timestamp', async () => {
      ...
      vi.mocked(prisma.security_alerts.update).mockResolvedValue(mockUpdatedAlert as any);
      ...
      fc.asyncProperty(
        ...
        async (eventInputs) => {
          ...
          for (let i = 0; i < results.length; i++) {
            expect(results[i].tenant_id).toBe(eventInputs[i].tenant_id);
            expect(results[i].terminal_id).toBe(eventInputs[i].terminal_id);
            expect(results[i].event_type).toBe(eventInputs[i].event_type);
          }
