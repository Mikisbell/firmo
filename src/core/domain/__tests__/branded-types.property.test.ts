...
describe('Branded Types - Property Tests', () => {
  ...
  describe('Type Safety Invariants', () => {
    ...
    it('TerminalId values are non-empty strings', () => {
      testInvariant(
        terminalIdArb,
        (id) => typeof id === 'string' && (id as string).length > 0 && (id as string).length <= 20,
        'TerminalId must be non-empty string ≤20 chars'
      );
    });
