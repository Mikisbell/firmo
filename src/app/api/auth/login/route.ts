...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use explicit mac_address or fallback to pseudo-MAC generated by frontend, or deviceFingerprint
    const rawEffectiveMac = data.mac_address || deviceFingerprint || '00:00:00:00:00:00';
    const effectiveMac = rawEffectiveMac.substring(0, 17); // Ensure it fits VarChar(17)
    ...
    if (data.terminal_id && !isAdmin) {
      ...
      log.debug('ADMIN bypass - omitiendo validación de terminal pero registrando acceso');
      
      // Auto-authorize MAC for this terminal so others can use it
      if (effectiveMac && terminalUuid) {
        await checkTerminalAuthorization(
          data.tenant_id,
          terminalUuid,
          effectiveMac,
          authResult.employee!.id
