/**
 * PedidosYa Webhook - POST
 *
 * Public endpoint (no admin auth). Receives order notifications from PedidosYa.
 * Validates HMAC-SHA256 signature before processing.
 *
 * @module app/api/webhooks/pedidosya/route
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await service.handleIncomingOrder(tenantId, 'PEDIDOSYA', payload, signature);
