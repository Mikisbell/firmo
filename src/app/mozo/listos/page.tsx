            ...
            <div className="p-3 md:p-6 space-y-4 pb-24 md:pb-8">
                ...
                {readyNotifications.length === 0 && checkNotifications.length === 0 ? (
                    <div className="flex flex-col items-center justify-center min-h-[50vh] text-center p-8">
                        ...
                        <p className="text-zinc-600 text-sm">Cuando cocina marque items listos, aparecerán aquí</p>
                    </div>
                ) : (
                    <>
                        <AnimatePresence mode="popLayout">
                            {readyNotifications.map((notif) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`p-3 rounded-xl flex-shrink-0 ${notif.read ? 'bg-zinc-700/30 text-zinc-500' : 'bg-emerald-500/20 text-emerald-400'}`}>
                                                <ChefHat className="w-6 h-6" />
                                            </div>

                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2 mb-0.5">
                                                    <span className={`font-bold text-base truncate ${notif.read ? 'text-zinc-400' : 'text-white'}`}>
                                                        {notif.itemName}
                                                    </span>
                                                    {!notif.read && (
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap text-xs text-zinc-500">
                                                    <span className="font-semibold text-zinc-300">Mesa {notif.tableNumber}</span>
                                                    {notif.station && (
                                                        <span className="px-2 py-0.5 bg-zinc-800 rounded font-medium uppercase tracking-wide">
                                                            {notif.station}
                                                        </span>
                                                    )}
                                            ...
                                            {!notif.read ? (
                                                ...
                                                <span className="flex-shrink-0 text-xs text-zinc-600 font-medium">Servido</span>
                                            )}
                                        </div>
                                    </motion.div>
                                );
                                ...
                                <AnimatePresence mode="popLayout">
                                    {checkNotifications.map((notif) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className={`p-3 rounded-xl flex-shrink-0 ${notif.read ? 'bg-zinc-700/30 text-zinc-400' : 'bg-amber-500/20 text-amber-400'}`}>
                                                        <CreditCard className="w-6 h-6" />
                                                    </div>
                                                    <div className="flex-1 min-w-0 text-left">
                                                        <p className={`font-bold text-base ${notif.read ? 'text-zinc-400' : 'text-amber-300'}`}>
