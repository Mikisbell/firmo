/**
 * PRUEBA de NOTA DE CREDITO (07) contra SUNAT BETA con el generador propio.
 * Uso: bun prisma/cleanup/verify-sunat-nc.ts
 */
import { readFileSync } from 'node:fs';
import { generateCreditNoteXml, type UblNotaCredito } from '@/src/core/integrations/sunat/sunat-ubl';
...
async function main() {
  ...
  const xml = generateCreditNoteXml(nc);
  ...
  const res = await client.sendBill('20000000001-07-FC01-1', signed.signedXml);

  if (res.accepted) {
    console.log('\n✅✅✅ NOTA DE CREDITO ACEPTADA POR SUNAT BETA');
    console.log('   responseCode:', res.responseCode, '| description:', res.description);
  } else {
    console.log('\n❌ NO aceptada');
